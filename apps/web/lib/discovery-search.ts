...
export async function searchCreators(filters: DiscoveryFilters, publicOnly = false) {
  ...
  if (filters.available) where.push(Prisma.sql`p.available=true`);
  if (filters.q)
  ...
  const price = filters.format
    ...
    : Prisma.sql`NULLIF(LEAST(COALESCE(r."postRate",2147483648),COALESCE(r."storyRate",2147483648),COALESCE(r."reelRate",2147483648),COALESCE(r."youtubeLongRate",2147483648),COALESCE(r."youtubeShortRate",2147483648)),2147483648)`;
