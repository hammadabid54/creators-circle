...
export function rankingSql(filters: DiscoveryFilters) {
  const freshAfter = new Date(Date.now() - 30 * 86400 * 1000);
  const now = new Date();
  ...
  const cte = Prisma.sql`WITH fresh_accounts AS (
    ...
    AND s."lastSyncedAt" BETWEEN ${freshAfter} AND ${now}
  ), engagement_peers AS (
    ...
    SELECT p.id "creatorId",
      ...
      + 5*LEAST(3::int,(SELECT count(*)::int FROM "PortfolioItem" x WHERE x."creatorId"=p.id AND (x.url LIKE 'https://%' OR x.url LIKE 'http://%')))
      ...
      + CASE WHEN length(trim(COALESCE(p.city,'')))>0 AND EXISTS(SELECT 1 FROM "CreatorTaxon" t JOIN "DiscoveryTaxon" d ON d.id=t."taxonId" WHERE t."creatorId"=p.id AND d.kind='niche' AND d.enabled=true) THEN 5 ELSE 0 END
      + CASE WHEN EXISTS(SELECT 1 FROM "CreatorTaxon" t JOIN "DiscoveryTaxon" d ON d.id=t."taxonId" WHERE t."creatorId"=p.id AND d.kind='language' AND d.enabled=true) THEN 5 ELSE 0 END
    ...
    SELECT *,readiness + 15*(CASE WHEN available THEN 1 ELSE 0 END) + 10*fresh + 5*engagement + 8 + 12.0*LEAST(brands::int,5)/5 score
