  ...
  const campaign = await db.campaign.findFirst({
    ...
    include: {
      applications: {
        include: {
          creator: {
            select: { name: true, image: true, creatorProfile: { select: { city: true, slug: true } } },
          },
        },
              ...
              {campaign.applications.map((a) => (
                <article className="cc-panel p-5 md:p-6" key={a.id}>
                  <div className="flex justify-between flex-wrap items-start gap-3">
                    <Link className="flex gap-3 items-center" href={'/creators/' + (a.creator.creatorProfile?.slug || a.creatorId)}>
                      <Avatar
